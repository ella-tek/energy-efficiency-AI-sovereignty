"""
Square matmul on GPU, N = 4096. 300 s after 10 warm-up calls, both excluded
...
def main():
    ...
    print(f"OFFICIAL GEMM run {args.run}  |  {host}  |  N={args.n}  B={args.b}")
    ...
    samples = calls   # one sample = one complete matmul
    occ_after = occupancy()
    measured_calls = float(counter.item())

    rec = {
        "run": args.run, "node": host, "kernel": "gemm",
        "N": args.n, "residency": "device_resident", "dtype": args.dtype,
