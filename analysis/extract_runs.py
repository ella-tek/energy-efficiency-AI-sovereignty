...
Pairs each timing record with its power trace and averages power over the
marker-gated region only. On a two-GPU node the busier card is used.

  python3 extract_runs.py <host> <ipu|gpu> > raw_<host>.csv
"""
...
# calls is the sample count: one sample is one kernel invocation. total_samples
# is carried through as well, since older GEMM records counted output rows.
w.writerow(["node", "kernel", "precision", "run", "mean_power_w",
            "calls", "total_samples", "total_time_s", "device", "power_rows",
            "idle_w"])

for kern in KERNELS:
    for dt in DTYPES:
        # careful: the power CSV drops the precision suffix for fp32 but the
        # json keeps it, so the two names do not match
        pats = [f"{kern}{dt}_{HOST}_run*_log.json"]
        if dt == "fp32":
            pats.append(f"{kern}_{HOST}_run*_log.json")   # round-1 legacy name
        seen = set()
        rows = []
        for pat in pats:
            for jf in sorted(glob.glob(os.path.join(D, pat))):
                ...
                if mw is None:
                    continue
                seen.add(run)
                rows.append([HOST, kern, dt, run, f"{mw:.4f}",
                             j["calls"], j["total_samples"],
                             f"{j['total_time_s']:.4f}",
                             dev, n, j.get("idle_w", "")])
